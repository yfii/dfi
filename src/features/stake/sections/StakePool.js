...
export default function StakePool(props) {
    ...
    const getPoolShare = () => {
        return myCurrentlyStaked.toNumber() > 0 ? ((Math.floor(myCurrentlyStaked.toNumber() * 10000)/ 10000) / poolsInfo[index].staked * 100).toFixed(4) : 0;
    }
    ...
    const customBgImg = (img) => {
        return img ? {backgroundImage: 'url(' + require('../../../images/' + img) + ')', backgroundSize: 'cover', backgroundRepeat: 'no-repeat'} : {}
    }

    return (
        <Grid container>
            <Grid item xs={6} className={classes.mb}>
                <Button href="/stake" className={classes.roundedBtn}>{t('Stake-Button-Back')}</Button>
            </Grid>
            <Grid item xs={6} className={classes.mb}>
                <Typography className={classes.countdown}>{poolsInfo[index].status === 'closed' ? 'FINISHED' : 'End: ' + myHalfTime}</Typography>
            </Grid>

            <Grid container className={[classes.row, poolsInfo[index].status === 'closed' ? classes.retired : ''].join(' ')}>
                <Grid item xs={6} sm={6} md={3}>
                    <Avatar src={require('../../../images/' + pools[index].logo)} alt={pools.earnedToken} variant='square' imgProps={{ style: { objectFit: 'contain' } }}/>
                </Grid>
                <Grid item xs={6} sm={6} md={3}>
                    <Typography className={classes.title}>{`${Math.floor(myBalance.toNumber() * 10000)/ 10000} ${pools[index].token}`}</Typography>
                    <Typography className={classes.subtitle}>{t('Stake-Balancer-Your-Balance')}</Typography>
                </Grid>
                <Grid item xs={6} sm={6} md={3}>
                    <Typography className={classes.title}>{`${Math.floor(myCurrentlyStaked.toNumber() * 10000)/ 10000} ${pools[index].token}`}</Typography>
                    <Typography className={classes.subtitle}>{t('Stake-Balancer-Current-Staked')}</Typography>
                </Grid>
                <Grid item xs={6} sm={6} md={3}>
                    <Box display="flex" justifyContent={'center'}>
                        <Typography className={classes.title}>{`${Math.floor(myRewardsAvailable.toNumber() * 10000)/ 10000} ${pools[index].earnedToken}`}</Typography>
                        <Avatar className={classes.fire} src={require('../../../images/stake/fire.png')} />
                    </Box>
                    <Typography className={classes.subtitle}>{t('Stake-Balancer-Rewards-Available')}</Typography>
                </Grid>
            </Grid>
            ...
            <Grid container className={[classes.row, poolsInfo[index].status === 'closed' ? classes.retired : ''].join(' ')}>
                ...
                <Grid item xs={12} sm={4}>
                    <Typography className={classes.title}>{getDailyReward()} {pools[index].earnedToken}</Typography>
            ...
            <Grid container className={classes.row}>
                ...
                <Grid item xs={12} md={6} lg={3}>
                    {isNeedApproval ? (
                        ...
                        <Button
                            ...
                            onClick={() => {handleModal(true, 'stake')}}>
                ...
                <Grid item xs={12} md={6} lg={3}>
                    <Button
                        ...
                        onClick={()=>{handleModal(true, 'unstake')}}>
                ...
                <Grid container className={classes.row} style={customBgImg(pools[index].partner.background)}>
                    ...
                    <Grid item xs={12}>
                        ...
                        {pools[index].partner.social.twitter ? (
                            <Link href={pools[index].partner.social.twitter}><TwitterIcon /></Link>
                        ) : ''}
                        {pools[index].partner.social.telegram ? (
                            <Link href={pools[index].partner.social.telegram}><TelegramIcon /></Link>
                        ) : ''}
            ...
            <Dialog onClose={() => {handleModal(false)}} aria-labelledby="customized-dialog-title" open={open}>
                ...
                <Grid container className={classes.modal}>
                    ...
                    <Grid item xs={12}>
                        <Typography className={classes.balance} onClick={() => {setInputVal(showInput === 'stake' ? myBalance.toString(): myCurrentlyStaked.toString())}}>
                            {pools[index].token} Balance: {showInput === 'stake' ? myBalance.toString(): myCurrentlyStaked.toString()}
                        </Typography>
                    </Grid>
                    <Grid item xs={12}>
                        <form noValidate autoComplete="off">
                            <TextField className={classes.input} value={inputVal} onChange={changeInputVal} autoFocus />
                        </form>
                    </Grid>
                    <Grid item xs={12} className={classes.modalbtns}>
                        <Button onClick={() => handleModal(false)} className={classes.actionBtn}>{t('Stake-Button-Back')}</Button>
                        <Button
                            className={[classes.actionBtn, pools[index].partnership && showInput === 'stake' ? classes.btnBoost : ''].join(' ')}
