...
export function fetchPairPriceOut(amountString, poolIndex, tokenIndex) {
  return (dispatch, getState) => {
    ...
    const promise = new Promise((resolve, reject) => {
      ...
      console.log('~~~~~ 11 ~~~~~',erc20Tokens[name].tokenContractAddress);
      console.log('~~~~~ 22 ~~~~~',outToken);
      console.log('~~~~~ 33 ~~~~~',amount);
      contract.methods.out(erc20Tokens[name].tokenContractAddress, outToken, amount).call({ from: address }).then(
        data => {
          console.log('~~~~~ fetchPairPriceOut ~~~~',data);
          ...
          console.log('~~~~~ fetchPairPriceOut newData ~~~~',newData);
          dispatch({
            type: LIQUIDITY_FETCH_PAIR_PRICE_OUT_SUCCESS,
            data: newData,
            poolIndex, 
