...
const _zapDeposit = ({ contract, address, vaultAddress, isETH, tokenAddress, tokenAmount, swapAmountOutMin, dispatch }) => {
  let transaction;
  ...
  if (isETH) {
    ...
    console.log('beefIn(vaultAddress, swapAmountOutMin, tokenAddress, tokenAmount)', vaultAddress, swapAmountOutMin, tokenAddress, tokenAmount);
    transaction = contract.methods.beefIn(vaultAddress, swapAmountOutMin, tokenAddress, tokenAmount).send({
      from: address,
    })
  }
