...
import {
  FARM_FETCH_EXIT_BEGIN,
  FARM_FETCH_EXIT_SUCCESS,
  FARM_FETCH_EXIT_FAILURE,
} from './constants';
    ...
    const promise = new Promise(async (resolve, reject) => {
      ...
      contract.methods.exit().send({ from: address }).on(
        ...
        .on('receipt', function(receipt){
          ...
          dispatch({ type: FARM_FETCH_EXIT_SUCCESS, index });
          resolve();
        })
        .on('error', function(error) {
          ...
          dispatch({ type: FARM_FETCH_EXIT_FAILURE, index });
          resolve();
        })
        .catch((error) => {
          dispatch({ type: FARM_FETCH_EXIT_FAILURE, index });
...
export function useFetchExit() {
  ...
  const { fetchExitPending } = useSelector(
    state => ({
      fetchExitPending: state.farm.fetchExitPending,
    })
  );
