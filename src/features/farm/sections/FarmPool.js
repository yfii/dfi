    ...
    <Grid container style={{ paddingTop: '4px', marginBottom: 200 }}>
      <Grid item xs={12}>
        ...
        <div className={classes.detailDesc}>
          ...
          <a href={earnedTokenUrl} target={'_blank'} style={{ color: 'rgb(54,85,152)' }}>
            {t('Farm-Know')} {earnedToken}
          </a>
        </div>
      </Grid>
      <Grid item xs={12}>
        {canStake ? <div className={classes.detailTime}>{t('Farm-Will-Mining-Over')}</div> : null}
        <div className={classes.detailTime} style={{ fontSize: 18, marginTop: 5 }}>
        ...
        <GridItem sm={6}>
          ...
          >
            <div className={classes.menuItemLogo}>
              <Avatar className={classes.menuItemLogoImg} src={require(`../../../images/${earnedToken}-logo.png`)} />
            </div>
            ...
            <Button className={classes.menuItemButton} disabled={!Boolean(claimAble)} onClick={onClaim}>
              {t('Farm-Reward')}
            </Button>
          </div>
        </GridItem>
        <GridItem sm={6}>
          ...
          >
            <div className={classes.menuItemLogo}>
              {isLP && lpTokens.length === 2 ? (
                lpTokens.map((item, index) => {
                  return <Avatar key={index} src={require(`../../../images/${item}-logo.png`)} className={classes.menuItemLogoImg} style={index > 0 ? offsetImageStyle : {}} />;
                })
              ) : (
            ...
            {!isNeedApproval ? (
              ...
              <Button className={classes.menuItemButton} disabled={!Boolean(approvalAble)} onClick={onApproval}>
            ...
            {!isNeedApproval ? (
              <>
                <Button
                  className={classNames({ [classes.menuItemButton]: true, [classes.menuItemButtonExit]: true })}
                ...
                >
                  {t('Farm-UnApproval')} {tokenDescription}
                </Button>
                <Button className={classNames({ [classes.menuItemButton]: true, [classes.menuItemButtonExit]: true })} disabled={!Boolean(exitAble)} style={{ bottom: -150 }} onClick={onExit}>
                  {t('Farm-Exit')}
                </Button>
              </>
            ) : null}
      ...
      >
        ...
        <DialogContent>
          ...
          <Input placeholder={t(isStake ? 'Farm-Stake-Amount' : 'Farm-UnApproval-Amount')} value={inputVal} fullWidth={true} classes={{ root: classes.dialogInput }} onChange={changeInputVal} />
        </DialogContent>
        <DialogActions style={{ height: 50 }}>
          <TextButton className={classes.dialogAction} style={{ color: '#F03278' }} disabled={!Boolean(isStake ? stakeAble : withdrawAble)} onClick={isStake ? onStake : onWithdraw}>
            {t('Farm-Dialog-Confirm')}
          </TextButton>
          <TextButton className={classes.dialogAction} style={{ color: '#000000' }} onClick={closeDialog}>
