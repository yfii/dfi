...
pools.forEach(pool => {
  ...
  if (pool.earnedTokenAddress !== pool.earnContractAddress) {
    messages.push(
      `Error: ${pool.id} : Pool earnedTokenAddress not same as earnedContractAddress: ${pool.earnedTokenAddress}, ${pool.earnedContractAddress}`
    );
    valid = false;
  }
  ...
  if (!pool.name.toLowerCase().includes(pool.token.toLowerCase())) {
    messages.push(
      `Warning: ${pool.id} : Pool name not same token type as token: ${pool.name}, ${pool.token}`
    );
  }
